...
{
    public class ReadSensorDataService_Old : IReadSensorDataService
    {
        private readonly IReadSensorDataRepository_Old _repository;
        private readonly ILogger<ReadSensorDataService_Old> _logger;

        public ReadSensorDataService_Old(IReadSensorDataRepository_Old repository, ILogger<ReadSensorDataService_Old> logger)
        {
        ...
        public async Task<ProcessedStatsDto> ProcessFileAsync(string filePath, CancellationToken cancellationToken = default)
        {
            ...
            foreach (var line in File.ReadLines(filePath))
            {
                ...
                if (string.IsNullOrWhiteSpace(line))
                {
                    ...
                    _logger.LogWarning("Line {LineNumber}: Empty or whitespace line", lineIndex + 1);
                    lineIndex++;
                    continue;
                }
                ...
                try
                {
                    ...
                    {
                        processed.IncrementInvalidRecords();
                        _logger.LogWarning("Line {LineNumber}: Deserialization returned null", lineIndex + 1);
                        lineIndex++;
                        continue;
                    }

                    // اعتبارسنجی اولیه مقادیر با هدف پرفورمنس و عدم ایجاد رکورد جدید در صورت داده نامعتبر
                    if (string.IsNullOrEmpty(dto.DeviceId) || string.IsNullOrEmpty(dto.Metric))
                    {
                        processed.IncrementInvalidRecords();
                        _logger.LogWarning("Line {LineNumber}: Missing DeviceId or Metric", lineIndex + 1);
                        lineIndex++;
                        continue;
                    }
                    ...
                    else
                    {
                        _logger.LogDebug("Line {LineNumber}: Duplicate reading detected: {ReadingId}",
                            lineIndex + 1, reading.Id);
                    }
                }
                catch (JsonException ex)
                {
                    processed.IncrementInvalidRecords();
                    _logger.LogWarning(ex, "Line {LineNumber}: JSON parsing error: {Line}", lineIndex + 1, line);
                }
                catch (Domain.Exceptions.InvalidReadingException ex)
                {
                    processed.IncrementInvalidRecords();
                    _logger.LogWarning(ex, "Line {LineNumber}: Invalid reading data", lineIndex + 1);
                }
                catch (Exception ex)
                {
                    processed.IncrementInvalidRecords();
                    _logger.LogError(ex, "Line {LineNumber}: Unexpected error processing line", lineIndex + 1);
                }
            ...
            var result = new ProcessedStatsDto()
            {
                ...
                ReadingsStored = processed.Readings.Count,
            ...
            _logger.LogInformation("  Total Lines: {TotalLines}", processed.TotalLines);
            _logger.LogInformation("  Readings Stored: {Stored}", processed.Readings.Count);
            _logger.LogInformation("  Duplicates Removed: {Duplicates}", processed.DuplicatesRemoved);
            _logger.LogInformation("  Invalid Records: {Invalid}", processed.InvalidRecords);
